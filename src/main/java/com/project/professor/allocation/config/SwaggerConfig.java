...
public class SwaggerConfig {
	...
	//Essas configurações foram só

	@Bean
    public Docket api() {
        return new Docket(DocumentationType.SWAGGER_2).select()
                .apis(RequestHandlerSelectors.basePackage(ProfessorAllocationApplication.class.getPackage().getName())) //Jeito programatico de dizer que eu quero todos os Controllers do pacote principal 
                .paths(PathSelectors.any()) // Quais são as URLS dos Controllesrs
                .build()
                .useDefaultResponseMessages(false)
                // A seguir Quais os métodos HTTP que eu vou utilizar 
